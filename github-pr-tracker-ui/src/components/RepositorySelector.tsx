...
interface RepositorySelectorProps {
  onRepositorySelect: (owners: string[], repos: string[]) => void;
}
...
const RepositorySelector: React.FC<RepositorySelectorProps> = ({ onRepositorySelect }) => {
  ...
  const handleSubmit = () => {
    if (selectedOwners.length > 0 && selectedRepos.length > 0) {
      onRepositorySelect(selectedOwners, selectedRepos);
    }
  };
  ...
  return (
    <Box sx={{ 
      ...
      <Button 
        ...
        disabled={selectedOwners.length === 0 || selectedRepos.length === 0}
        sx={{
          py: 1.5,
          textTransform: 'none',
          fontWeight: 600,
